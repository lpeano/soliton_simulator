...
for %%S in (1 2) do (
  for %%C in (30 100 300) do (
    START "elast_c%%C_s%%S" /MIN cmd /c "python %SIM% --batch --nmasse 3 --sep 4 --seed %%S --passi %PASSI% --ogni %OGNI% --verlet --sync --viriale --zeta-vir --pav-com --chi-basc --polo-maturo --olon-part --calore-vett --elast-c %%C --sync-db db\elast\db_c%%C_s%%S.pkl --db-ogni %DBOGNI% --csv csv\elast\cond_c%%C_s%%S.csv --diaglog csv\elast\c%%C_s%%S.csv > log\elast\elast_c%%C_s%%S.log 2>&1"
  )
)
