...
for %%S in (%SEMI%) do (
  START "batch_kf0_s%%S" /MIN cmd /c "python %SIM% --batch --nmasse %NM% --sep %SEP% --seed %%S --passi %PASSI% --ogni %OGNI% --sync --kfrange 0   --csv csv\test\cond_kf0_s%%S.csv --diaglog csv\test\diag_kf0_s%%S.csv > log\test\batch_kf0_s%%S.log 2>&1"
  START "batch_kf_s%%S"  /MIN cmd /c "python %SIM% --batch --nmasse %NM% --sep %SEP% --seed %%S --passi %PASSI% --ogni %OGNI% --sync --kfrange %KF% --csv csv\test\cond_kf_s%%S.csv  --diaglog csv\test\diag_kf_s%%S.csv  > log\test\batch_kf_s%%S.log 2>&1"
)
...
echo.
echo === Lanciati. ===
echo     csv\test\  : cond_*.csv (condensazione) + diag_*.csv (diaglog = la PROVA, Lz_orb)
echo     out_video\ : video_*.mp4 (camera fissa)
echo     log\test\  : output per run (se qualcosa non parte, guarda qui)
