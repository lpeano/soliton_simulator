...
| **❓ SCENA-1** | **`SEMINA_LAM` è approvata ma INCOMPATIBILE con la scena di DEFAULT del driver:** `semina(900)` in raggio `4.0` **RIFIUTA**, saturazione vera **`455`**. | **DA DECIDERE (Luca).** Le strade che vedo, e **nessuna è mia**: ① il driver passa `--nodi 0` di default *(ma allora ogni scena diversa dalla `(ii)` resta senza vuoto)*; ② si riduce `--nodi` *(ma la capienza **dipende dal seme**: `12807/12783/12812/12790`, ed è l'errore già preso)*; ③ `SEMINA_LAM` resta **esclusa** dal driver e si accende **solo** sulle scene compatibili. **Il criterio `S5` del sigillo del driver MISURA il conflitto a ogni corsa**, quindi il giorno in cui la scena cambia lo dice da sé. |
