...
| **REG-B** | **FASE B: le SCHEDE**, a lotti, **un commit per lotto** | MANDATO-REGISTRO §2 | ▶ **`doc/REGISTRO_FISICA.md` ESISTE**, con la struttura, `REG-R` dichiarata *(non cablata)*, e **la SCHEDA ① del FRENO DI `SCALA_MIN`**: forma dal codice, invariante verificato *(`LAM` e' una BARRIERA ASSORBENTE dall'alto)*, dimensioni, siti, **`A11` corollario per corollario** *(cor.4 e cor.7b VIOLATI)*, e **i TRE CANDIDATI come DOMANDE**. **ORDINE:** ① freno ✅ · ② **memoria del moto** · ③ gravita' bifase · ④ coesione · poi mitosi, Schwinger, `peq`; poi le altre attive, poi le dormienti |
