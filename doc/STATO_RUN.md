...
| **G4-bis** | **IL SECONDO BRACCIO: spegnere l'INTERO blocco di `mem_mot`**, spostamento di fase compreso | richiesta di Luca, 2026-09-22 | ⚠ **SIGILLO `8/10` — e i DUE FAIL sono nei CRITERI, non nel flag** *(reperto committato)*. ✅ `T9` **byte-inerte acceso: `206` campi identici, `0` diversi** · ✅ `T4` `mem_mot` **identicamente zero** spento · ✅ `T5` `phi` **differisce**: il ramo di `:6033` era raggiunto · ✅ `T8` `MEM_HEBB=False` ne tocca **sei**. ❌ `T7` contava i **punti** mentre l'AST conta i **rami** *(`3` rami coprono `4` punti)* · ❌ `T6` pretendeva `phi` identica, **ma lo spostamento di fase LEGGE `d0`** |
