...
| **G4-bis** | **IL SECONDO BRACCIO: spegnere l'INTERO blocco di `mem_mot`**, spostamento di fase compreso | richiesta di Luca, 2026-09-22 | ▶ **VIA LIBERA DI LUCA.** ✅ **flag `MEM_MOTO_TUTTO`** *(costante di modulo, `True` di default, nessun flag CLI; blob `ab685eac` → `21e3a3dc`)* — **la SECONDA E ULTIMA modifica al simulatore ammessa prima del `CHK3`**. Recinta i **quattro** punti: `:5655` l'aggiornamento · `:5657` `memedge` e `proj` · `:5670` `S08_proj` · **`:6033` la proiezione trasversale → `phi`**. ⏳ **sigillo scritto e committato, DA GIRARE** |
