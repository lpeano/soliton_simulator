...
| **REG-B** | **FASE B: le SCHEDE**, a lotti, **un commit per lotto** | MANDATO-REGISTRO §2 | ▶ **`doc/REGISTRO_FISICA.md`: DUE schede su quattro.** ① **freno di `SCALA_MIN`** ✅ *(forma, invariante, `A11` corollario per corollario, i TRE CANDIDATI come domande)* · ② **memoria del moto** ✅ *(forma, **le dimensioni non tornano: `proj` e' adimensionale e si somma a una lunghezza**, l'ipotesi della compressione MISURATA — `Z112`)* · ③ gravita' bifase ⏸ · ④ coesione ⏸ · poi mitosi, Schwinger, `peq`; poi le altre attive, poi le dormienti |
