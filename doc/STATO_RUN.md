...
| **G4-bis** | **IL SECONDO BRACCIO: spegnere l'INTERO blocco di `mem_mot`**, spostamento di fase compreso | richiesta di Luca, 2026-09-22 | ⏸ **ATTIVATO da `Z109` e NON ANCORA PARTITO.** **PRIMA serve il suo SIGILLO** *(pattern ⑤: nessuna prova di spegnimento senza involucro dimostrato)*, **e il flag non esiste ancora.** **COSA DEVE COPRIRE, letto dal sorgente:** ① `:5655` l'aggiornamento `mem_mot = (1-plast)*mem_mot + plast*grad_tw` · ② `:5657-5658` `memedge` · ③ `:5670` il sito `S08_proj` *(gia' recintato da `MEM_MOTO`)* · **④ `:6033` `proiezione_trasversale = sum(mem_mot[ii]*dir_laterale)`, che diventa `shift_fase_dinamico` e scrive `self.phi[ii]`** — **l'effetto INDIRETTO che `MEM_MOTO` lascia vivo.** **⚠ NON si usa `MEM_HEBB = False`:** il `T6` del sigillo di `G3` ha MISURATO che toglie **cinque** siti oltre questi |
