...
| **S2** | **NUOVO (2026-09-16): il CONTRASTO ON-OFF su `chi` esclude lo zero.** `+0.1769 +- 0.0500`, **IC95 [+0.0177, +0.3362]** con `t(3)`. **NON e' la voce S** (quella chiedeva se `chi` si stacca dal casuale: no); questa dice che **`--tau-luce` sposta `chi` di +0.18 gradi**. **⚠ CONFONDENTE ENORME:** i due bracci differiscono anche in **risoluzione di un fattore ~7** (`theta` **91.5-105.2** contro **11.9-16.5** giri/passo) e **nessuno dei due e' risolto** | un terzo braccio a risoluzione **intermedia**: se il contrasto **scala con la risoluzione** e' campionamento, se resta costante e' fisica. Oppure un braccio con `theta < 1` giro/passo |
...
| **D** | rumore **bianco senza taglio spettrale** (catastrofe UV discreta) | **PREDIZIONE SCRITTA, CABLAGGIO NON FATTO — e la predizione e' CORRETTA, non trascritta** (2026-09-16, `doc/PREDIZIONE_taglio_spettrale.md`). La **giustificazione di principio REGGE** (un rumore bianco discreto inietta a tutte le frequenze fino a Nyquist; nessun rumore fisico lo fa; `tau_c = LAM/CS_M` e' **derivato**). **Ma tre affermazioni del mandato contraddicono fatti misurati:** **①** la ricorsione OU data **PRESERVA la varianza** (`b^2/(1-a^2) = 1.000000` analitico, `0.997330` simulato su 400 000 passi): **non riduce l'ampiezza di 4.47**, e ottenerlo richiederebbe di toccare `amp`, cioe' un coefficiente (§3) che il mandato stesso vieta — **le due prescrizioni sono incompatibili**; **②** il rumore **non muove il Bloch**: calcio **7.2-9.4 gradi/passo** contro un moto di **4 274-37 874 gradi/passo** da `omega`, cioe' **0.02-0.22 %** (misurato sui 4 run di oggi) — e sotto `--spinore-corretto` il `_nb` committato e' **derivato dallo spinore**, quindi il calcio viene **sovrascritto**; **③** il canale che resta e' gia' misurato **marginale**: **`R_stoc = 0.041`** contro l'errore atteso `0.097` (**C6**) | **PREDIZIONE CORRETTA: `theta` non scende. Se si muove, SALE** (≲5 %, perche' una forzante correlata su 40 passi fa crescere `omega` stocastico come `n` invece che `sqrt(n)` — ma su una componente che vale il 4 %). **Chiude:** cablare la legge **con l'attesa corretta**, **piu' un sigillo tipo S7** perche' il `dt` della ricorsione dev'essere **`dt_n = DT*r`**, non `DT` (il mandato non lo dice; e' l'errore gia' preso nello Strato 1, che solo S7 stano'). **Serve il via di Luca: quale delle due prescrizioni incompatibili vale** |
