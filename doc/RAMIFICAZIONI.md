...
| **S** | **`chi` materia è SOTTO 90 nel braccio OFF e SOPRA 90 nell'ON, segno concorde su entrambi i semi. Segnale o rumore?** | OFF **89.778 / 89.876** (media 89.827), ON **90.048 / 90.084** (media 90.066). Preso ingenuamente: `z ~ 3.5` in entrambe le direzioni. **Ma con 2 semi la dev.std ha UN grado di liberta', `t(0.025,1) = 12.706`, e l'IC95 è largo 1.2 gradi: contiene lo zero.** NON è stabilito | **≥ 4 semi per braccio** (`t(3) = 3.18`, quattro volte piu' stretto). Se il segno resta concorde e l'IC95 esclude 90, è **la prima firma non nulla del settore** |
