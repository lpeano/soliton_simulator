...
| **C19** | **⚠ DUE `theta` DIVERSI, e i numeri T3 gia' committati NON sono confrontabili con quelli di MISURA F.** `_rimisura_t3.py` `:73` usa `theta = \|omega\|*DT` (tempo di **COORDINATA**); MISURA F `:389` usa `theta = \|omega\|*dt_n` (tempo **PROPRIO**, quello fisicamente giusto per CLAUDE.md §9). **L'attesa `-0.69` e tutto C8 sono nella convenzione VECCHIA** | il braccio OFF si muove nello stesso verso (`-0.10 -> -0.075`) **senza che nulla lo giustifichi**: il segnale e' della convenzione, non della maturazione | si chiude scrivendo **entrambe** le convenzioni nello stesso run: **una riga**, zero run nuovi |
