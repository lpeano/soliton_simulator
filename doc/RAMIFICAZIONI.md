...
| **X2** | **`ZETA_LOC`: «smorzamento locale» che dipende da una statistica GLOBALE, e ha un PUNTO FISSO** | `med_rho = max(float(np.median(rho)), 1e-9)` (**`:3136`**) e' la mediana su **tutto** il sistema, poi `eccesso = max(rho/med_rho - 1, 0)` e `zeta_loc = ZETA_M/(1+eccesso)`. **Due difetti distinti:** (1) **non-localita'** — lo smorzamento di un arco dipende dalla densita' dell'intero sistema (§4); (2) **P4, ed e' il piu' insidioso:** per il nodo **mediano** `eccesso = 0` **sempre**, a qualunque maturazione, quindi `zeta_loc = ZETA_M` **per costruzione**. E' **lo stesso punto fisso auto-normalizzante** gia' registrato per `_tau = TAU_A*max(_dens/_dens_rif, 0.05)` (§9, voce **C12**): **sulla mediana non si misura nulla** | e' un **gauge legittimo** solo se si dimostra che la mediana funge da **unita' di scala** e non da accoppiamento. Il test e' quello di **C12**: due configurazioni a densita' diverse devono dare `zeta` mediano **diverso** — se danno lo stesso, e' il punto fisso e la «localita'» e' apparente |
