...
for _k, (_tp, _mot) in TIPO_A_MANO.items():
    if _k in VOCI:
        VOCI[_k]["tipo"] = _tp
        VOCI[_k]["nota"] = "tipo deciso a mano: " + _mot
...
def pulisci_campo(s):
    """Un campo del TSV non puo' contenere un TAB ne' un capo di riga: si sostituiscono.

    ⚠ Prima qui c'era una `re.sub` con una classe di caratteri scritta a mano, e in un mio
    patch script gli escape sono diventati **un TAB e un NEWLINE VERI dentro la stringa**:
    `SyntaxError`. **Un campo del TSV si ripulisce con `replace`, che non ha escape da rovinare.**
    """
    return (s or "").replace(TAB, " ").replace(NL, " ").replace(chr(13), " ")
...
for k in sorted(VOCI):
    v = VOCI[k]
    out.append(TAB.join([
        v["id"], ",".join(sorted(v["alias"])), pulisci_campo(v["titolo"]), v["fonte"],
        v["stato"], v["blocca"], v["tipo"], v["fam"], pulisci_campo(v.get("stato_da", "")),
        v.get("avanz", "(senza marcatore)"),
        ("%s#%s" % (REVISIONE_DOC, REVISIONE[v["id"]])) if v["id"] in REVISIONE else "",
        pulisci_campo(v.get("motivo", "")), pulisci_campo(v.get("nota", "")),
    ]))
