...
def main():
    ...
    W("3. **IL `93 %` NON REGGE SU QUESTI ARCHIVI, e lo dice la tabella qui sopra:** i nodi\n")
    W("   con `r < 0.1` sono fra lo **`0.30 %`** e il **`18.69 %`**, e la QUOTA DI TEMPO che\n")
    W("   sta nei fermi e' fra lo **`0.01 %`** e l'**`1.83 %`**. **La premessa di `D25` cade\n")
    W("   qui come quella di `D27`.** Resta la domanda, ma con un numero diverso: **una quota\n")
    W("   di tempo cosi' piccola basta a falsare la PROVA 3** *(tutti i corpi cadono uguale)*,\n")
    W("   **o e' trascurabile?** **Non e' deciso, e non lo decide questa misura.**\n")
