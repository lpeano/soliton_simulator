...
K4 IL CASO CHE IL CRITERIO VECCHIO SBAGLIAVA: due scritture nei DUE RAMI di un
     `if/else`, con UN solo `_traccia_d0` dopo. Ne gira UNA SOLA.
     criterio VECCHIO -> ne dichiara una scoperta (SBAGLIATO) ; criterio NUOVO -> le copre entrambe ; esito OK
------------------------------------------------------------------------------------------------
  -> i criteri PASSANO
...
| `6030` | `memoria_hebbiana_moto` | aumentato per indice | `S12_coesione` | `self.d0[mask] += self._sd0(_delta_coes, mask)` |
...
**Tutte le 2 non tracciate hanno una lettura che dice dove il bilancio le prende.**
**E il bilancio CHIUDE a `1.138e-13` e `1.170e-13`**, che e' la verifica *a posteriori* di questa tabella: **se una di queste letture fosse sbagliata, il bilancio non chiuderebbe.**
