...
TUTTE LE LEGGI SONO SEMPRE ATTIVE â€” nessun interruttore per la fisica:
...
# STATO nella successione discreta degli stati del sistema â€” un contatore, un tick. Il sistema
...
#
# ============================================================================
# LEGGE DELLA DENSITA' CRITICA â€” FORMA ADATTIVA ALLE SCALE (crossover di saturazione)
# ----------------------------------------------------------------------------
...
def scuoti_vuoto(net):
    ...
    L'intensitÃ  emerge dallo scostamento fra la distanza reale (d) e la distanza di riposo (d0)
    ...
    # 3. IntensitÃ  non-parametrica: radice dello stress locale modulata dalla coerenza
    # - Dove lo spazio Ã¨ teso/frustrato, lo scuotimento sale.
    # - Dove c'Ã¨ materia coerente (|Psi|^2 alto), viene soppresso dalla formula pura (1 + I2/Lam).
    intensita_stress = np.sqrt(stress_nodo + 1e-9)
    ampiezza = intensita_stress * (np.sqrt(Lam) / (1.0 + I2 / Lam))

    # Inietta l'agitazione vettoriale di fase firmata dalla chiralitÃ 
...
LS_AZIM = False         # LÂ·S VETTORIALE (legge): se True, il verso tangenziale della
...
CHI_DA_SPINORE = False  # FLAG 3 (separato, NON nel master): perc_chi = segno di doppia-copertura di _psi_spinor
                        # DOPO il commit di psi, e CHI_BASC disattivato. RICHIEDE --spinore-corretto (sennÃ² loop).
...
SYNC_FASE_OROLOGIO = False # KURAMOTO SUL SEGNO DI DOPPIA-COPERTURA (la via genuina all'ordinamento del segno,
                        # Â§43): torque RELAZIONALE O(dt^1) che tira la fase di doppia-copertura alpha_k =
...
OROLOGIO_SEGNO = False    # MOD 5.3c (dev-spinoriale): firma il VERSO dell'orologio de Broglie INTERNO _phc col
                        ...
                        # Richiede --campo-spinoriale + --spinore-corretto. Default off = byte-identico.
KURAMOTO_SU2 = False     # KURAMOTO SU(2) NON-ABELIANO (Â§46): ruota lo SPINORE INTERO verso la media SU(2) dei
...
class Rete:
    ...
    def _aggiorna_lift_spinoriale(self, nb_precedente=None):
        ...
        Non definisce da solo una cucitura spaziale MÃ¶biusiana; rende pero' misurabile il segno
    ...
    def _link_su2(nb_i, nb_j):
        ...
        DESTINAZIONE D'USO â€” "due oggetti, due usi" (presidio di Luca):
        ...
        det N=0, quindi U = 0/0 e' indefinita â€” non esiste una rotazione che porti n_j su -n_j.
    ...
    def chiralita_core_locale(self):
        """Calcola la chiralitÃ  emergente del core locale per ogni nodo."""
    ...
    def circolazione_topologica(self):
        ...
        # (ricostruisce lo spinore dal Bloch -> CIECA al segno-orologio, Â§38-bis), questa usa il PRIMARIO
    ...
    def _passo_spinoriale(self, i, j, w, dt_n, psi_snapshot=None,
        ...
        if SPINORE_CORRETTO:
            ...
            if SYNC_FASE_OROLOGIO and forza_sync is not None and wI_sync is not None and uno_sync is not None:
                # KURAMOTO SUL SEGNO DI DOPPIA-COPERTURA (sync-fase-orologio, Â§43): ordina il foglio +- in modo
            ...
            if KURAMOTO_SU2 and forza_sync is not None and wI_sync is not None:
                # KURAMOTO SU(2) NON-ABELIANO (Â§46): ruota lo SPINORE INTERO verso la media SU(2) dei vicini,
    ...
    # avvolgimento su Â±4pi: permette alla torsione di vivere sul dominio DOPPIO (4pi)
    ...
    def step(self):
        ...
        # Congeliamo lo stato iniziale affinchÃ© tutti i calcoli leggano i campi sincronizzati,
        ...
        if TORS_4PI and len(self.perc_chi) >= self.n:
            ...
            if POLO_MATURO:
                ...
                twist_dip = np.pi * 0.5 * (chi_torsione[i] - chi_torsione[j])  # chiralitÃ  core o nodale
        ...
        if HAM_SRC == 0.0:
            ...
            # inizializzato Ã¨ il dato di t. Gli altri usano esclusivamente lo snapshot.
    ...
    def mitosi(self):
        ...
        # (fino a 2pi) piu' i due mezzi-twist dipolari (Â±pi) = 3pi come quanto naturale,
    ...
    def memoria_hebbiana_moto(self):
        """MEMORIA HEBBIANA DELLA DINAMICA â€” come LEGGE, non parametri.
        ...
        if len(self.tw) and len(self.i) and self.n > 0:
            ...
            if mask.any():
                ...
                # 1. IntensitÃ  di campo locale e media di riferimento relativa
                ...
                # Limite geometrico causale del passo di fase per preservare la stabilitÃ  del campo
...
def bussola(ax, R, M, scala=0.13):
    ...
    all'immagine â€” decorazione confondibile con struttura, per giunta sopra la
...
TESTS = {
 "VUOTO": [
   dict(cap="VUOTO 1/2 â€” solo puntatori scorrelati: nessuna materia stabile.\n"
            "Le interferenze fluttuano senza mai addensarsi", dur=110),
   dict(cap="VUOTO 2/2 â€” il vuoto respira ma NON deriva: con le onde sulla\n"
            "deformazione, a riposo la geometria e' immobile per costruzione", dur=110)],
 "LIBERO": [
   dict(cap=lambda: f"OSSERVAZIONE LIBERA 1/6 â€” il sistema intero, senza interventi. "
                    f"puntatori {net.n}, archi {len(net.i)}", dur=250),
   dict(cap=lambda: f"2/6 â€” coerenza globale {net.diagnostica()['coer_g']:.3f}, "
                    f"locale {net.diagnostica()['coer_l']:.3f}; d medio {net.diagnostica()['d_med']:.3f}",
        dur=250),
   dict(cap=lambda: f"3/6 â€” dilatazione {100*net.diagnostica()['dil']:+.1f}%, "
                    f"stress {100*net.diagnostica()['stress']:+.2f}%: la geometria si assesta?",
        dur=250),
   dict(cap=lambda: f"4/6 â€” torsione max {net.diagnostica()['tw']:.1f}/{PHI_CRIT:.1f}; "
                    f"mitosi {net.nati}", dur=250),
   dict(cap=lambda: f"5/6 â€” picco |Psi|Â² {net.diagnostica()['picco']:.1f}; "
                    f"la materia si organizza in strutture stabili?", dur=250),
   dict(cap=lambda: f"6/6 â€” bilancio: d medio {net.diagnostica()['d_med']:.3f}, "
                    f"coerenza locale {net.diagnostica()['coer_l']:.3f}, mitosi {net.nati}",
        dur=250)],
 "MASSA": [
   dict(cap="MASSA 1/3 â€” nasce un dominio COERENTE dentro il vuoto (sotto N_c)",
        al_via=lambda: _massa(0.0, _size_video(0, 1.5), int(massa_critica_collasso()*0.45), 0.0, "massa"), dur=120),
   dict(cap="MASSA 2/3 â€” la materia si addensa dove le fasi si accordano", dur=140),
   dict(cap=lambda: f"MASSA 3/3 â€” dilatazione {100*net.diagnostica()['dil']:+.1f}%: "
                    "il pozzo relazionale attorno alla materia", dur=140)],
 "BUCO NERO": [
   dict(cap="BUCO NERO 1/3 â€” dominio OLTRE la densita' critica N_c",
        al_via=lambda: _massa(0.0, _size_video(0, 1.2), int(massa_critica_collasso()*1.8), 0.0, "buconero"), dur=120),
   dict(cap="BUCO NERO 2/3 â€” oltre soglia i gusci si omogeneizzano: la materia collassa", dur=140),
   dict(cap=lambda: f"BUCO NERO 3/3 â€” coerenza uniforme {net.diagnostica()['coer_l']:.3f}: "
                    "il regime omogeneo, i gusci annullati", dur=140)],
 "BIG BANG": [
   dict(cap="BIG BANG 1/4 â€” evento iniziale denso e CALDO: l'universo si accende",
        al_via=lambda: _big_bang(calore=0.5, punti=1500), dur=60),
   dict(cap=lambda: f"BIG BANG 2/4 â€” creazione di materia (nodi {net.n}): la mitosi genera",
        dur=140),
   dict(cap=lambda: f"BIG BANG 3/4 â€” espansione e diluizione: TAU_TW cala, "
                    f"la generazione rallenta (nodi {net.n})", dur=200),
   dict(cap=lambda: f"BIG BANG 4/4 â€” universo assestato con la materia creata "
                    f"(nodi {net.n}, dil. {100*net.diagnostica()['dil']:+.0f}%)", dur=220)],
 "SISTEMA": [
   dict(cap="1/5 â€” BUCO NERO massiccio al centro (3x N_c)",
        al_via=lambda: _massa((0.0,0.0,0.0), 1.6, int(massa_critica_collasso()*3.0), 0.0, "buconero"),
        dur=110),
   dict(cap="2/5 â€” tre MASSE strutturate poste LONTANE, ben separate",
        al_via=lambda: [_massa(p, 0.8, int(massa_critica_collasso()*0.35), f, "massa_%d"%k)
                        for k,(p,f) in enumerate([((5.2,0.6,0.0),0.4),
                                                  ((-3.0,4.4,0.5),1.9),
                                                  ((-2.6,-4.6,-0.6),3.3)])],
        dur=140),
   dict(cap="3/5 â€” il campo d'interferenza si tende FRA le masse e il centro",
        dur=200),
   dict(cap=lambda: f"4/5 â€” deformazione del campo: dilatazione {100*net.diagnostica()['dil']:+.1f}%, "
                    f"stress {100*net.diagnostica().get('stress',0):+.1f}%",
        dur=200),
   dict(cap=lambda: f"5/5 â€” evoluzione lunga: come si deforma il campo fra i corpi "
                    f"(coerenza {net.diagnostica()['coer_l']:.2f})",
        dur=260)],
 "DUE-MASSE": [
   dict(cap="DUE MASSE 1/3 â€” prima massa coerente (0.8x N_c) a sinistra",
        al_via=lambda: _massa(-_sep_video(), _size_video(0, 0.7), int(massa_critica_collasso()*0.8), 0.0, "massaA"), dur=15),
   dict(cap="DUE MASSE 2/3 â€” seconda massa (0.8x N_c) a destra, vicina ma non unita",
        al_via=lambda: _massa(_sep_video(), _size_video(1, 0.7), int(massa_critica_collasso()*0.8), 0.0, "massaB"), dur=15),
   dict(cap=lambda: f"DUE MASSE 3/3 â€” frame-dragging attivo: osservare se la congiungente "
                    f"ruota (precessione) e se i solitoni corrono lungo le geodetiche",
        dur=220)],
 "N-MASSE": [
   dict(cap=lambda: f"N MASSE 1/2 â€” {_n_masse_video()} masse in cerchio (raggio {_sep_video():.1f}), "
                    f"ben separate; lo zoom segue lo scaling dello spazio",
        al_via=lambda: _semina_n_masse(), dur=120),
   dict(cap=lambda: f"N MASSE 2/2 â€” evoluzione libera: condensazione fra le masse e guscio esterno; "
                    f"coerenza {net.diagnostica()['coer_l']:.2f}, dilatazione {100*net.diagnostica()['dil']:+.1f}%",
        dur=280)],
 "TERRA-BUCONERO": [
   dict(cap="1/4 â€” un oggetto COLLASSATO (2.5x N_c): il 'buco nero'",
        al_via=lambda: _massa(-2.2, _size_video(0, 1.5), int(massa_critica_collasso()*2.5), 0.0, "buconero"), dur=110),
   dict(cap="2/4 â€” una piccola MASSA strutturata (0.12x N_c): la 'Terra', a distanza",
        al_via=lambda: _massa(3.0, _size_video(1, 0.7), int(massa_critica_collasso()*0.12), 0.0, "terra"), dur=130),
   dict(cap="3/4 â€” i rapporti coi rispettivi N_c sono dalla legge di densita' critica",
        dur=150),
   dict(cap=lambda: f"4/4 â€” la massa presso l'oggetto collassato "
                    f"(dil. tempo {100*net.diagnostica()['dil']:+.1f}%): rappresentazione qualitativa",
        dur=170)],
 "TERRA-SOLE": [
   dict(cap="TERRA-SOLE 1/4 â€” un oggetto grande e coerente (il 'Sole'): massa "
   ...
   dict(cap="TERRA-SOLE 2/4 â€” un piccolo oggetto (la 'Terra') a distanza orbitale, "
   ...
   dict(cap="TERRA-SOLE 3/4 â€” con il coarse-graining (avvia con --scala) l'orbita "
            "vive a piu' lunghezze d'onda: la gravita' compete con l'espansione",
        dur=170),
   dict(cap=lambda: f"TERRA-SOLE 4/4 â€” moto della 'Terra' presso il 'Sole' "
                    f"(dil. tempo {100*net.diagnostica()['dil']:+.1f}%): "
                    f"osservare se orbita, cade o si allontana", dur=200)],
 "URTO": [
   dict(cap="URTO 1/4 â€” una massa coerente si assesta nel vuoto relazionale",
        al_via=lambda: _massa(0.0, _size_video(0, 1.5), 420, 0.0, "massa"), dur=120),
   dict(cap="URTO 2/4 â€” CARTUCCIA densa in anti-fase accanto alla massa",
        al_via=lambda: (test["dati"].__setitem__("d0", net.diagnostica()["d_med"]),
                        _massa(2.6, _size_video(1, 0.9), 320, np.pi, "cartuccia")), dur=150),
   dict(cap=lambda: f"URTO 3/4 â€” la geometria si dilata: d medio "
 ...
 "SCONTRO": [
   ...
   dict(cap="SCONTRO 2/4 â€” ACCRESCIMENTO fra le due: ciascuna riceve materiale\n"
        ...
        dur=140),
   dict(cap="SCONTRO 3/4 â€” i centri MIGRANO verso l'interno per ricomposizione\n"
        ...
        dur=140),
   dict(cap=lambda: f"SCONTRO 4/4 â€” incontro: materia A x{_energia('A'):.2f}, "
                    f"B x{_energia('B'):.2f}; d medio {net.diagnostica()['d_med']:.2f}",
        dur=130)],
 "FILA RADIALE": [
   dict(cap="FILA RADIALE â€” massa centrale R3 e 5 masse R1 in fila (centri 6,10,14,18,22)",
        al_via=_semina_fila_radiale, dur=200),
   dict(cap="FILA RADIALE â€” evoluzione libera: le masse cadono, restano, o si espandono?", dur=300),
   dict(cap=lambda: f"FILA RADIALE â€” d medio {net.diagnostica()['d_med']:.2f}; "
                    "osservare il campo fra centrale e satelliti (la luna) e le posizioni (il dito)", dur=400)],
 "TRE MASSE D3": [
   dict(cap="TRE MASSE â€” triangolo a distanza 3, tutte le leggi + momento angolare fisico",
   ...
   dict(cap="TRE MASSE â€” l'interferenza si accumula nello spazio fra le masse (la luna)", dur=250),
   dict(cap=lambda: f"TRE MASSE â€” d medio {net.diagnostica()['d_med']:.2f}; "
                    "il campo comune al centro cresce prima che le masse si muovano", dur=300)],
 "BUCO NERO": [
   dict(cap="BUCO NERO â€” 1 massa centrale e 10 masse attorno a distanza 4.5",
        al_via=_semina_buco_nero, dur=150),
   dict(cap="BUCO NERO â€” le masse esterne rispondono al pozzo centrale", dur=170),
   dict(cap=lambda: f"BUCO NERO â€” d medio {net.diagnostica()['d_med']:.2f}; "
                    "momento angolare conservato: evoluzione orbitale attorno al centro", dur=200)],
 "DUE MASSE": [
   dict(cap="DUE MASSE 1/3 â€” due domini coerenti fra loro, affiancati",
        al_via=lambda: (_massa(-_sep_video(), _size_video(0, 1.2), 320, 0.0, "A"), _massa(_sep_video(), _size_video(1, 1.2), 320, 0.0, "B")), dur=130),
   dict(cap="DUE MASSE 2/3 â€” le interferenze si cercano nello spazio fra loro", dur=150),
   dict(cap=lambda: f"DUE MASSE 3/3 â€” d medio {net.diagnostica()['d_med']:.2f}; "
                    f"materia A x{_energia('A'):.2f}, B x{_energia('B'):.2f}: "
                    "si attraggono, o la geometria si limita a dilatarsi?", dur=150)],
 "ANTIFASE": [
   dict(cap="ANTIFASE 1/2 â€” due domini in OPPOSIZIONE di fase",
        al_via=lambda: (_massa(-_sep_video(), _size_video(0, 1.2), 320, 0.0, "A"), _massa(_sep_video(), _size_video(1, 1.2), 320, np.pi, "B")), dur=140),
   dict(cap=lambda: "ANTIFASE 2/2 â€” misura: dominio A x%.2f, dominio B x%.2f.\n"
                    "Dove le fasi si oppongono l'interferenza si cancella, "
                    "benche' i puntatori ci siano tutti" % (_energia("A"), _energia("B")), dur=170)],
 "TERRA-BH": [
   dict(cap="TERRA presso BUCO NERO 1/4 â€” a sinistra materia strutturata (sotto N_c), "
   ...
   dict(cap="TERRA presso BUCO NERO 2/4 â€” la materia mantiene i suoi gusci; "
            "il collasso si omogeneizza in un blocco coerente", dur=150),
   dict(cap=lambda: f"TERRA presso BUCO NERO 3/4 â€” pozzo del collassato molto piu' profondo; "
                    f"la materia (x{_energia('terra'):.2f}) sente la geometria del buco nero (x{_energia('bh'):.2f})", dur=150),
   dict(cap=lambda: f"TERRA presso BUCO NERO 4/4 â€” d medio {net.diagnostica()['d_med']:.2f}; "
...
_y0 = 0.135 if "--test" not in _sys.argv else 0.03      # spazio per i bottoni: interattivo sÃ¬, video no
...
def _render_vista_rete_sola(net, M, R, dg):
    ...
    # La scala appartiene al grafo, non al campo: cosÃ¬ l'evoluzione topologica resta leggibile.
    ...
    ax.set_title("EVOLUZIONE DEL GRAFO PLANARE Â· pozzo fisico\n"
                 f"frame {stato['nframe']} Â· N={n} Â· E={len(net.i)} Â· colore = phi_g",
    ...
    ax3d.set_axis_off()
    ax3d.set_title("RETE DINAMICA 3D Â· pozzo fisico\ncolore nodi = phi_g",
    ...
    ax2.set_title("RETE DINAMICA Â· pendenza del pozzo\ncolore archi = |Î”phi_g|",
                  fontsize=9, color="#0277bd")

    # Pannello metriche: stessi indicatori principali della vista campo.
    righe = [
        ("VISTA RETE", 14, "#0277bd", "bold"),
        ("stessa dinamica Â· stesso stato", 9, "#7b1fa2", "normal"), ("", 8, "#000", "normal"),
        ...
        (f"phi_g: {phi_g.min() if n else 0:.3g} .. {phi_g.max() if n else 0:.3g}",
         10, "#7b1fa2", "normal"),
        (f"|Î”phi_g| max: {dphi_max:.3g}", 10, "#7b1fa2", "normal"),
        ("â€” GEOMETRIA VIVA â€”", 10, "#7b1fa2", "bold"),
    ...
    axc.text(0.5, 0.5, "VISTA RETE Â· premere N o usare --vista-campo per tornare alla vista attuale",
...
def _render_grafo_topologico(net, dg):
    ...
    n = net.n
    if n == 0:
        ax.set_title("GRAFO TOPOLOGICO Â· vuoto", fontsize=11, color="#0277bd")
        return
    ...
    # Layout astratto: seleziona i nodi piÃ¹ connessi/densi solo per rendere leggibile
    # una rete enorme. La posizione sul pannello NON Ã¨ net.pos e non entra nella fisica.
    limite = 2800
    ...
    ax.set_aspect("equal"); ax.axis("off")
    ax.set_title("GRAFO TOPOLOGICO ASTRATTO Â· non XYZ\n"
                 f"frame {stato['nframe']} Â· nodi mostrati {len(sel)}/{n} Â· "
                 f"archi {len(net.i)} Â· colore = phi_g / |Î”phi_g|",
...
def update(frame):
    ...
    ax.set_title("LA SOLA INTERFERENZA (fondo incoerente sottratto)\n"
                 "fuoco = materia   Â·   ciano = distruzione   Â·   nero = nulla\n"
                 f"(volumetrico Â· camera {np.degrees(angolo_vista())%360:.0f}Â°"
                 f"{' auto' if stato['rot_auto'] else ' â€” trascina per ruotare'})",
    ...
    ax3d.axis("off")
    ax3d.set_title("POZZO FISICO DEL GRAFO\n(phi_g usato da GRAV_BIFASE Â· interpolazione grafica)",
    ...
    ax2.set_aspect("equal"); ax2.axis("off")
    ax2.set_title("rete dinamica Â· archi=twist Â· nodi=|Psi|Â²", fontsize=9, color="#444")
    ...
    T = [("MURATORE DI PLANCK", 14, "#111", "bold"),
         ...
         (f"puntatori: {net.n} / {MAX_NODI}", 10, "#111", "normal"),
         (f"archi:     {len(net.i)}", 10, "#111", "normal"), ("", 8, "#000", "normal"),
         ("â€” MATERIA (dal campo) â€”", 10, "#111", "bold"),
         ...
         ("", 8, "#000", "normal"), ("â€” GEOMETRIA VIVA â€”", 10, "#7b1fa2", "bold"),
         (f"d medio:     {dg['d_med']:.3f}", 10, "#7b1fa2", "normal"),
         (f"dilatazione: {100*dg['dil']:+.1f}%", 10, "#7b1fa2", "normal"),
         (f"stress:      {100*dg['stress']:+.2f}%", 10, "#7b1fa2", "normal"),
         ("", 8, "#000", "normal"), ("â€” TOPOLOGIA â€”", 10, "#0277bd", "bold"),
         (f"torsione max: {dg['tw']:.1f} / {(3*np.pi if TORS_4PI else PHI_CRIT):.1f}", 10, "#0277bd", "normal"),
         (f"mitosi: {net.nati}  (negate {net.negate})", 10, "#0277bd", "normal"),
    ...
    for txt, sz, col, wt in T:
        ...
        y -= 0.042 if txt else 0.018
    if stato["piazza"]:
        axc.text(0.5, 0.5, "â–¶ PIAZZAMENTO %s ATTIVO â€” clicca sul pannello 3D dove crearlo (raggio %.1f)"
...
def _applica_flag(a):
    ...
    CHI_CORE = bool(getattr(a, "chi_core", False)) # chiralitÃ  emergente del core locale
    ...
    if CHI_CORE:
        print("[chi-core] frame-dragging guidato dalla chiralitÃ  emergente del core locale")
    if CS_DINAMICO:
        print("[cs-dinamico] velocitÃ  metrica locale cs_eff(rho) con profilo tanh e CFL dinamico")
    ...
    if SCALA_B != 1.0:
        print(f"[scala] coarse-graining B={SCALA_B:.0f}: ogni solitone rappresenta "
              ...
              f"resta la stessa â€” i blocchi occupano lo spazio dei fini, piu' grossolanamente.")
...
def batch_condensazione(a):
    ...
    def _diag_completa(net, step):
        ...
        # segno_k = sign(Re<canon(nb_k)|psi_k>) = foglio Â±1 che l'orologio pura-fase pilota (riga ~2265):
        ...
        cols['verso_arco_coer'] = 0.0    # <nb_iÂ·nb_j> pesato: allineamento del verso di Bloch
        ...
        if (_nb_g is not None and _psp is not None and len(_psp) >= n and len(_nb_g) >= n and n > 0 and len(net.i)):
            try:
                ...
                if _mk.any():
                    ...
                    # PRESIDIO CRITICO (Â§5.1): coerenza del SEGNO di doppia-copertura _sgn DENTRO il settore
