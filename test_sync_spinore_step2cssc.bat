...
if not exist log\sync_spinore_step2cssc mkdir log\sync_spinore_step2cssc
...
for %%S in (1 2 3) do (
  ...
  python %SIM% --batch --nmasse 2 --sep %SEP% --seed %%S --passi %PASSI% --ogni %OGNI% %PREREQ% --sync-spinore --sync-db db\sync_spinore_step2cssc\db_on_s%%S.pkl --db-ogni %DBOGNI% --csv csv\sync_spinore_step2cssc\cond_on_s%%S.csv --diaglog csv\sync_spinore_step2cssc\on_s%%S.csv > log\sync_spinore_step2cssc\on_s%%S.log 2>&1
)
for %%S in (1 2 3) do (
  echo [step2 cs+tauloc+scuoti seed %%S] OFF
  python %SIM% --batch --nmasse 2 --sep %SEP% --seed %%S --passi %PASSI% --ogni %OGNI% %PREREQ% --sync-db db\sync_spinore_step2cssc\db_off_s%%S.pkl --db-ogni %DBOGNI% --csv csv\sync_spinore_step2cssc\cond_off_s%%S.csv --diaglog csv\sync_spinore_step2cssc\off_s%%S.csv > log\sync_spinore_step2cssc\off_s%%S.log 2>&1
)

echo.
echo RUN AGGIUNTIVO (chi-core+cs-dinamico+tauloc 2.0 + scuotimento) completato: db\sync_spinore_step2cssc\, csv\sync_spinore_step2cssc\, log\sync_spinore_step2cssc\.
