...
echo     Criterio: r_com finale (il collasso si ferma?). Log in .\log\vir\
echo.

REM ================= BATCH: la PROVA (CSV + diaglog) su TUTTI i semi =================
for %%S in (%SEMI%) do (
  START "voff_s%%S" /MIN cmd /c "python %SIM% --batch --nmasse %NM% --sep %SEP% --seed %%S --passi %PASSI% --ogni %OGNI% --sync --kfrange 0            --csv csv\vir\cond_voff_s%%S.csv --diaglog csv\vir\diag_voff_s%%S.csv > log\vir\voff_s%%S.log 2>&1"
  START "von_s%%S"  /MIN cmd /c "python %SIM% --batch --nmasse %NM% --sep %SEP% --seed %%S --passi %PASSI% --ogni %OGNI% --sync --kfrange 0 --viriale  --csv csv\vir\cond_von_s%%S.csv  --diaglog csv\vir\diag_von_s%%S.csv  > log\vir\von_s%%S.log 2>&1"
)

REM ================= VIDEO: l'OCCHIO (camera FISSA), seed 1, off + on =================
START "video_voff_s1" /MIN cmd /c "python %SIM% --test N-MASSE --nmasse %NM% --sep %SEP% --seed 1 --giri 0 --ppf 1 --frames %FRAMES% --fps %FPS% --size 0.7,0.7,0.7 --sync --kfrange 0           --out out_video\video_voff_s1.mp4 > log\vir\video_voff_s1.log 2>&1"
START "video_von_s1"  /MIN cmd /c "python %SIM% --test N-MASSE --nmasse %NM% --sep %SEP% --seed 1 --giri 0 --ppf 1 --frames %FRAMES% --fps %FPS% --size 0.7,0.7,0.7 --sync --kfrange 0 --viriale --out out_video\video_von_s1.mp4 > log\vir\video_von_s1.log 2>&1"

echo.
echo === Lanciati. Risultati in csv\vir\ (CSV+diaglog), out_video\, log\vir\ ===
