...
for %%S in (1 2 3) do (
  ...
  python %SIM% --batch --nmasse 2 --sep %SEP% --seed %%S --passi %PASSI% --ogni %OGNI% %PREREQ% --sync-spinore --sync-db out_sync_spinore_step2\db_on_s%%S.pkl --db-ogni %DBOGNI% --csv out_sync_spinore_step2\cond_on_s%%S.csv --diaglog out_sync_spinore_step2\on_s%%S.csv > log\sync_step2_on_s%%S.log 2>&1
)
for %%S in (1 2 3) do (
  echo [step2 chi-core seed %%S] OFF
  python %SIM% --batch --nmasse 2 --sep %SEP% --seed %%S --passi %PASSI% --ogni %OGNI% %PREREQ% --sync-db out_sync_spinore_step2\db_off_s%%S.pkl --db-ogni %DBOGNI% --csv out_sync_spinore_step2\cond_off_s%%S.csv --diaglog out_sync_spinore_step2\off_s%%S.csv > log\sync_step2_off_s%%S.log 2>&1
)

echo.
echo STEP 2 (chi-core) completato: 6 run in out_sync_spinore_step2\.
